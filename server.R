...
server <- function(input,output,session) {
  ...
  output$selectCity <- renderUI(
   if(is.null(input$State)) #TODO: Need to remove .. will create junk input
     selectInput(inputId ="city",label = "Choose a city:", choices= 
               as.character(uniqueCity))
   else{
      # Get the data set with the appropriate name
      cityForState <- sort(unique(business_df[business_df$State == input$State, "City"]))
      selectInput(inputId ="city",label = "Choose a city:", choices= as.character(cityForState))
  }
 ...
 observeEvent(input$searchButton, {
   ...
   #For RDs file
   tempDF <- business_df %>% filter(City == input$city & Sub_category == input$Type)
   ...
   if(exists("tempDF") == FALSE){
     tempDF <- business_df
     resultContent <- c("NA","NA","NA","NA","NA")
     resultsDF <- data.frame(resultRowID, resultContent)
     #resultsDF <- data.frame(resultRowID)
   }
   else{
    ...
    dummy = nrow(unique(tempDF$business_id))
    #Call MOdel pending
    resultContent <- c(totalResturant , avgRestaurantRatings , totalReviewCount , dummy , dummy)
